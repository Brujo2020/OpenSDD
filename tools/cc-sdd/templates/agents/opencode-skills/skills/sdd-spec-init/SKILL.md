...
3. **Check Uniqueness**: Verify `{{SDD_DIR}}/specs/` for naming conflicts. If the directory already exists with only `brief.md` (no `spec.json`), use that directory (discovery created it).
4. **Create Directory**: `{{SDD_DIR}}/specs/[feature-name]/` (skip if already exists from discovery)
