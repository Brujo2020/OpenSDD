...
const hookTemplate = path.join(repoRoot, 'tools', 'cc-sdd', 'templates', 'hooks', 'pre-commit.mjs');
const posixTemplate = path.join(repoRoot, 'tools', 'cc-sdd', 'templates', 'hooks', 'pre-commit');
const installerScript = path.join(repoRoot, 'scripts', 'install-hooks.mjs');
const cliEntry = path.join(repoRoot, 'tools', 'cc-sdd', 'dist', 'cli.js');
...
const makeCheckout = async (options: { git?: boolean } = {}): Promise<string> => {
  ...
  const templateDir = path.join(dir, 'tools', 'cc-sdd', 'templates', 'hooks');
