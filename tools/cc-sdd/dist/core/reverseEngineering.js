...
export const scanProject = async (cwd = process.cwd()) => {
    ...
    const workspaceGlobs = (() => {
        ...
        if (Array.isArray(declared))
            return declared.filter((w) => typeof w === 'string');
        if (declared && typeof declared === 'object' && Array.isArray(declared.packages)) {
            return (declared.packages).filter((w) => typeof w === 'string');
        }
        return [];
    })();
