...
export const runGate = async (gateId, ctx, regime = 'flexible') => {
    ...
    switch (gateId) {
        ...
        case 'C4': {
            ...
            await walk(path.join(ctx.cwd, 'tools', 'cc-sdd', 'src'));
        ...
        case 'C6': {
            const missing = [];
            const candidates = ['README.md', 'CLAUDE.md', 'package.json', `tools/cc-sdd/package.json`];
        ...
        case 'O1': {
            ...
            const lock = (await readIfExists(path.join(ctx.cwd, 'tools/cc-sdd/package-lock.json'))) !== null;
