...
export const buildModuleMap = async (cwd) => {
    ...
    const workspaceRoots = [...(project.workspaceRoots ?? [])].sort();
    const modulePaths = ['.', ...workspaceRoots];
    const notes = [];
    let complete = true;
    ...
    const manifestRel = (modulePath) => modulePath === '.' ? 'package.json' : `${modulePath}/package.json`;
