...
const runPivot = async (cwd: string, feature: string | undefined, sddDir: string | undefined): Promise<PivotRun> => {
    ...
    io.log(
      JSON.stringify(
        pivot.alignment
          ...
          : { ...report, alignmentError: pivot.error ?? 'el pivote no se pudo ejecutar', checkExitCode: pivot.exitCode },
        null,
        2,
      ),
    );
  ...
  if (isCheck) {
    ...
    if (!pivot.alignment) {
      ...
      if (alignment.findings.length === 0) {
        io.log(
          `  ${colors.green('✓')} ${alignment.detail.split(';')[0]}: sin hallazgos.`,
        );
      }
      ...
      if (alignment.findings.some((finding) => finding.severity === 'error')) exitCode = 1;
    }
  }
