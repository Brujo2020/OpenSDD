...
const installAgentSkills = async (
  ...
  if (cliPath === null) {
    return {
      action: 'keep',
      failure:
        'se pidió --skills pero no se encontró el instalador (CLI) alcanzable: la instalación del agente NO se ha ejecutado. Instálalo con `npx open-sdd@latest --version` o compílalo con `npm --prefix tools/cc-sdd run build`.',
    };
  }
