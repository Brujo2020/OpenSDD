...
export const scanProject = async (cwd: string = process.cwd()): Promise<DiscoveredProject> => {
  ...
  if (await checkExists('package.json')) {
    try {
      ...
      if (await checkExists('pnpm-lock.yaml')) packageManager = 'pnpm';
      else if (await checkExists('yarn.lock')) packageManager = 'yarn';
      else if (await checkExists('bun.lockb')) packageManager = 'bun';
    ...
    language = 'Python';
    if (await checkExists('poetry.lock')) packageManager = 'poetry';
    else if (await checkExists('uv.lock')) packageManager = 'uv';
