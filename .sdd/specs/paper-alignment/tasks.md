...
`_Depends:_` and `_Boundary:_` are parsed by `tools/cc-sdd/src/core/specManager.ts` and must be the
...
- [x] 1. Restore the deleted product from git history and confirm the baseline — _Requirements: 1_ — _Boundary:_ `.`_
  ...
  - _Evidence: `npm --prefix tools/cc-sdd run build` → exit 0 (Ensured shebang and exec bit on …/dist/cli.js); `npm --prefix tools/cc-sdd test` → 47 files, 233 tests passed, 1.46s._
...
- [x] 2. Fix the recursive, failure-masking root postinstall — _Requirements: 1_ — _Boundary:_ `package.json`, `scripts`_
  ...
  - _Evidence: `scripts/postinstall.mjs` prints "workspace tools/cc-sdd is not present; skipping dependency install" and exits 0; replica without tools/ finished in 248ms, zero child npm install processes (previously 81 concurrent after 8s)._
...
- [x] 4. Encode the 21-gate taxonomy and the executable chain — _Requirements: 2, 3_ — _Boundary:_ `tools/cc-sdd/src/core/gateCatalog.ts`_
...
- [x] 5. Resolve the chain from catalog, profile and signals — _Requirements: 2_ — _Depends:_ `4`_ — _Boundary:_ `tools/cc-sdd/src/core/gateCatalog.ts`_
...
- [x] 6. Compute the crosswalk residue by subtraction — _Requirements: 3_ — _Depends:_ `4`_ — _Boundary:_ `tools/cc-sdd/src/core/gateCatalog.ts`_
...
- [x] 7. Model enforcement levels, per-host ceilings and the behavioural sentinel — _Requirements: 5_ — _Boundary:_ `tools/cc-sdd/src/core/enforcement.ts`_
...
- [x] 8. Implement the default-FAIL posture with the hard subset — _Requirements: 4_ — _Depends:_ `7`_ — _Boundary:_ `tools/cc-sdd/src/core/enforcement.ts`_
...
- [x] 9. Encode invariants I1–I6 and conformity C0–C3 — _Requirements: 6_ — _Boundary:_ `tools/cc-sdd/src/core/invariants.ts`_
...
- [x] 10. Add the relaxation ledger and the appeal channel — _Requirements: 6_ — _Boundary:_ `tools/cc-sdd/src/core/receipts.ts`_
...
- [x] 11. Add quantified HITL thresholds and rigor selection — _Requirements: 6_ — _Boundary:_ `tools/cc-sdd/src/core/hitl.ts`_
...
- [x] 12. Implement the EARS validator and the Triad/evidence-lock checks — _Requirements: 4_ — _Boundary:_ `tools/cc-sdd/src/core/ears.ts`, `tools/cc-sdd/src/core/triad.ts`_
...
- [x] 13. Implement transactional waves and the scope contract — _Requirements: 7_ — _Boundary:_ `tools/cc-sdd/src/core/waves.ts`_
...
- [x] 14. Implement META-EVAL, memory anti-poisoning and the skills privilege rule — _Requirements: 8, 9_ — _Boundary:_ `tools/cc-sdd/src/core/metaEval.ts`, `tools/cc-sdd/src/core/memory.ts`, `tools/cc-sdd/src/core/skills.ts`_
...
- [x] 15. Add telemetry, budgets, the claims registry and assurance mappings — _Requirements: 10_ — _Boundary:_ `tools/cc-sdd/src/core/telemetry.ts`, `tools/cc-sdd/src/core/claims.ts`, `tools/cc-sdd/src/core/assurance.ts`_
...
- [x] 16. Wire the governance console into the CLI — _Requirements: 2, 3, 10_ — _Boundary:_ `tools/cc-sdd/src/cli/commands`, `tools/cc-sdd/src/index.ts`_
...
- [x] 17. Add the regression suite for the new modules — _Requirements: 2, 3, 4, 5, 6, 7, 8, 9, 10_ — _Depends:_ `4`, `8`, `10`, `13`, `14`_ — _Boundary:_ `tools/cc-sdd/test`_
  ...
  - _Evidence: `npm --prefix tools/cc-sdd test` → 62 files, 470 tests passed; 15 new test files added (236 new tests) alongside the 233 pre-existing ones._

- [x] 20. Fix the defects the new suite exposed — _Requirements: 2, 4, 6, 8, 9_ — _Depends:_ `17`_ — _Boundary:_ `tools/cc-sdd/src/core`_
...
- [x] 18. Align documentation with the implementation and publish the traceability report — _Requirements: 10_ — _Depends:_ `16`_ — _Boundary:_ `README.md`, `docs`, `CLAUDE.md`, `AGENTS.md`, `CHANGELOG.md`_
  ...
  - _Evidence: `README.md` rewritten (real bin entries, 18 agent definitions, both profile axes, console, "what this port does not do"); `docs/PAPER-ALIGNMENT.md` (383→400 lines) records the G1–G21 → C1–C7/O1–O7 → crosswalk → residue tables, a ~30-row component map with symbols and status labels, and 14 numbered gaps G-01…G-14; `docs/QUICK-START.md` and `docs/INSTALLATION.md` created; `CLAUDE.md`/`AGENTS.md` corrected (templates under `tools/cc-sdd/templates/agents/<agent>/skills/`, 160 SKILL.md, prefix `sdd-`); `CHANGELOG.md` extended. Stale strings (`src/cli/index.ts`, "9 Production-Ready", `.claude/skills/` as a source path) verified absent as fact._

- [x] 21. Make the claims registry executable and the repository pass its own gate — _Requirements: 4, 10_ — _Depends:_ `15`, `16`_ — _Boundary:_ `tools/cc-sdd/src/core/claimsRegistry.ts`, `tools/cc-sdd/test/coreClaimsRegistry.test.ts`, `.sdd/specs/paper-alignment/requirements.md`_
  ...
  - _Evidence: `node tools/cc-sdd/dist/cli.js assure claims --verify` → "35 afirmaciones | 31 verificadas | 0 declaradas | 3 no medidas | 0 rotas | 1 desactualizadas", exit 0; `node tools/cc-sdd/dist/cli.js gates run` → "La cadena pasa", exit 0. Two defects were found and fixed on the way: the runner used a login shell (`bash -lc`) whose profile can change the working directory, and the YAML parser unescaped backslashes inside single-quoted scalars — together they reported 17 working controls as broken. The two genuinely compound requirements in this spec were split, and the trigger rule was tightened to clause starts so ordinary English ("passes where the text declared an absence") is not flagged._

- [x] 22. Expose the §9.7 decidable discipline and stop the console from being misread — _Requirements: 4_ — _Depends:_ `15`, `16`_ — _Boundary:_ `tools/cc-sdd/src/cli/commands/paper.ts`, `tools/cc-sdd/src/core/triad.ts`_
...
- [x] 26. Unify the release identity and protect it with guards — _Requirements: 1, 10_ — _Depends:_ `18`, `23`_ — _Boundary:_ `package.json`, `tools/cc-sdd/package.json`, `.github/workflows/publish.yml`, `docs/INSTALLATION.md`_
  ...
  - _Evidence: `publish.yml` now runs build + test + `gates run` + `assure claims --verify`, asserts `@brujo2020/open-sdd`, and publishes from the root with `--access public --provenance`; `tools/cc-sdd/package.json` is `private: true`; `install:global` installs the root; 12 documentation install commands corrected across 5 files. 10 new tests in `test/releaseIntegrity.test.ts`. Writing the guards exposed a real parser bug: `claimsRegistry.ts` unescaped YAML double-quoted strings in sequence, so `\\n` became a backslash plus a newline and split a verifier across lines — fixed with a single-pass unescape, found by a new claim failing as `broken`. Suite 65 → 66 files, 500 → 510 tests; claims 40 → 43, 0 broken._
