...
> `tools/cc-sdd/src/core/triad.ts`).
...
| ADR-BF-004 | Reconnaissance is workspace-aware and configuration-aware, and reports `unknown` rather than guessing. | Proven necessary by running the survey on this repository: it reported "JavaScript, no tests detected" on a TypeScript project with 513 tests, because the code lives in `tools/cc-sdd`. |
...
1. `npm --prefix tools/cc-sdd run build` and `test` green.
