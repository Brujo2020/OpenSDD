...
\begin{itemize}
    ...
    \item \textbf{Autonomous}: Decentralized QE agents automatically generate metamorphic test cases, property-based invariants, and characterization suites for legacy brownfield monoliths without requiring manual test scripting.
...
By deploying Agentic QE via the Model Context Protocol (MCP), Open-SDD provides an enterprise-grade validation layer that directly drives the bypass rate $d(\phi) \to 0$, providing concrete, mathematical assurance for EU AI Act Article 14 human oversight and NIST AI RMF MAP/MEASURE functions.
...
\section{Brownfield Enterprise Modernization: Code-First Spec Reverse-Engineering}

Deploying agentic SDD across legacy codebases within enterprise environments and hyperscalers (AWS, GCP, Azure) requires avoiding brittle full-rewrite traps. Open-SDD operationalizes brownfield adoption through a code-first bootstrap protocol (\texttt{/sdd-getspecs}) and a repeatable four-phase onboarding pipeline:

\begin{enumerate}
    \item \textbf{Code-First Reverse Engineering (\texttt{sdd-getspecs})}: The harness inspects existing source code, configuration files, and git commit history to bootstrap project steering (\texttt{product.md}, \texttt{tech.md}, \texttt{structure.md}) and generate an acyclic roadmap of candidate specifications.
    \item \textbf{Unapproved Spec Seeds with Mandatory Human Validation Gate}: For each identified boundary, \texttt{sdd-getspecs} materializes a \textit{spec seed} containing a brownfield brief, an initialized descriptor (\texttt{phase: "initialized"} with all approvals set to \texttt{false}), and a requirements stub. Crucially, Open-SDD prohibits jumping directly from code extraction to generative implementation; human engineers must edit, calibrate, and validate the deduced requirements via \texttt{/sdd-spec-requirements} and verify boundary blast radiuses with \texttt{/sdd-validate-gap} before design approval.
    \item \textbf{Golden Characterization Locking}: Legacy endpoints are bounded by automated characterization suites and Agentic QE metamorphic property tests, capturing existing runtime behaviors and edge-case invariants before modification.
    \item \textbf{Transactional Wave Refactoring (\texttt{sdd-impl})}: Implementation tasks execute in isolated subagent waves behind feature flags. If tests violate the golden characterization baseline or spec contracts, the task is rejected and the worktree is rolled back cleanly.
\end{enumerate}
